...
#include "pa_set_volume_from_stdin.hpp"
 ...
 */
void AudioBackend::subscribeCb(pa_context* context, pa_subscription_event_type_t type, uint32_t idx, void* data) {
 ...
 */
void AudioBackend::sinkInfoCb(pa_context* /*context*/, const pa_sink_info* i, int /*eol*/, void* data) {
    if (i == nullptr)
        return;
    ...
    backend->default_sink_running_ =
        backend->default_sink_name == i->name && (i->state == PA_SINK_RUNNING || i->state == PA_SINK_IDLE);
    ...
    if (!backend->current_sink_running_ && (i->state == PA_SINK_RUNNING || i->state == PA_SINK_IDLE)) {
    ...
    if (backend->current_sink_name_ == i->name) {
        ...
        float volume        = static_cast<float>(pa_cvolume_avg(&(backend->pa_volume_))) / float{PA_VOLUME_NORM};
 ...
 */
void AudioBackend::sourceInfoCb(pa_context* /*context*/, const pa_source_info* i, int /*eol*/, void* data) {
    auto* backend = static_cast<AudioBackend*>(data);
...
void AudioBackend::run() {
    pa_mainloop_run(this->mainloop_, nullptr);
}

int main() {
    auto backend = AudioBackend::getInstance();
